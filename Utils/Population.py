...
def NewPlayer(_rating: float, _hidden: float=0.0, _scv: float=None, _iscv: bool = False) -> Player:
    if _scv:
        mu = ln(_rating)
        return _iscv and Player(_rating, _hidden, _scv*mu, mu) or Player(_rating, _hidden, _scv, mu) 
...
def Populate(_N: int, _SCV: int = 0)->list:
    match _SCV:
        case 0:
            return [NewPlayer(START_RATING, int(1300 + 200*RNG.random())) for _ in range(_N)]
        case 1:
            return [NewPlayer(START_RATING, int(1300 + 400*RNG.random()), START_SIGMA) for _ in range(_N)]
        case 2:
...
def Results(_pop:list, _matches: int, _BTM: bool=False):
    def generateBTM(_pop: list, _matches: int)->list:
        length = len(_pop)
    ...
    def generateELO(_pop: list, _matches: int)->list:
        length = len(_pop)
        results = RNG.random(size=(_matches, length, 3))
        return [[GetIndex(r[0], length), GetIndex(r[1], length),
                ...
                for result in results for r in result]
    if _BTM:
        return [item for item in generateBTM(_pop, _matches) if item[0] != item[1]]
    return [item for item in generateELO(_pop, _matches) if item[0] != item[1]]
